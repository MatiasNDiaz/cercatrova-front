...
/**
 * Escena 3D del loader — la casita sobre la isla flotante.
 *
 * ── Por qué vive en su propio archivo ───────────────────────────────────────
 * `three` es un motor 3D completo (~130 kB en el bundle). Estaba importado de
 * forma ESTÁTICA dentro de `Loadingpage.tsx`, que a su vez es lo que renderiza
 * `(public)/loading.tsx`. Como ese `loading.tsx` es la frontera de Suspense del
 * segmento `(public)`, el chunk de three.js entraba en el **First Load JS de
 * todas las rutas públicas**: `/`, `/properties`, `/properties/:id`,
 * `/servicios/:id` y `/publicaciones` — las páginas que el visitante ve
 * primero, y para una animación decorativa que en una navegación rápida no
 * llega a verse nunca.
 *
 * El padre lo carga con `next/dynamic({ ssr: false })` y sólo después de que se
 * abre el gate de aparición, así que el navegador descarga three.js únicamente
 * cuando la escena se va a mostrar de verdad.
 *
 * `ssr: false` no es opcional: la escena construye un `WebGLRenderer`, que
 * necesita `window` y un `<canvas>` reales.
 *
 * ⚠️ El cuerpo del efecto es el MISMO de antes, movido tal cual. Lo único que
 * cambió es el gate: antes el efecto dependía de `visible` y salía temprano
 * mientras estaba cerrado; ahora ese control lo hace el padre, que directamente
 * no monta este componente hasta que corresponde.
 */

/** Ancho máximo de la escena. Por debajo de esto se achica con el viewport. */
const SCENE_MAX_W = 420;
/** Alto de la escena como proporción del ancho. */
const SCENE_RATIO = 0.8;

export default function Escena3D({ className = '' }: { className?: string }) {
