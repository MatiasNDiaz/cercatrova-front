...
**Un admin en `/dashboard` ya no es un bug de UX: es una feature.** Hay context-switcher explícito en los dos sentidos — el sidebar admin tiene "Vista de Usuario" (👁 → `/dashboard`) y el sidebar de usuario muestra "Panel Admin" (🛡 → `/dashboardAdmin`) **solo si `user.role === 'admin'`**. Fue una decisión, no un olvido.
...
- **Sidebar de `/dashboard`** (`DashboardShell` + `Sidebar` local): "Volver al inicio", **Inicio**, grupo **Mis propiedades** (Favoritos / Que valoré / Que comenté), grupo **Preferencias**, grupo **Notificaciones** (6 filtros por `?tipo=`), **Mis solicitudes**, sección Cuenta (Editar Perfil / Preferencias), y logout.
- **Rutas:** `/dashboard`, `/dashboard/{favoritos, valoradas, comentadas, mis-solicitudes, notificaciones, perfil, preferencias}` y `/publicar`.

### Admin (`role: 'admin'`)
- **Sidebar de `/dashboardAdmin`:** **Inicio**, grupo **Propiedades** (publicar / gestionar / ver catálogo público), grupo **Publicaciones** (crear / gestionar), grupo **Notificaciones** con badges por tipo (6 subrutas reales, una página por categoría), grupo **Solicitudes** (`?estado=`), grupo **Usuarios** (`?rol=`), sección Cuenta (Mi Perfil / **Estadísticas**), "Vista de Usuario" y logout.
...
- ⚠️ El sidebar admin (`dashboardAdmin/layout.tsx`) **sigue trayendo la lista completa** de `GET /notifications/admin` cada 60s, porque necesita el desglose por tipo (usuarios/solicitudes/comentarios/valoraciones/favoritos) para los badges del menú, y `unread-count` solo devuelve el total. La clasificación por tipo se hace con `getNotifType(title, message)`, que **matchea substrings del texto en español** ("usuario registrado", "solicitó", "valoración", "comentó", "favorito"). Es frágil: si el backend cambia el copy de una notificación, el badge de esa categoría deja de contar sin que nada falle.
